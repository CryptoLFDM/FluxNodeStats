...
func CalCulRentability(nodes Nodes, stats FluxBlocsStats) []byte {
	var RentabilityNodes []Rentability
	for _, NodeList := range NodesList {
		...
		RentabilityNodes = append(RentabilityNodes, RentabilityNode)
	}
	RentabilityNodesJson, _ := json.Marshal(RentabilityNodes)
	return RentabilityNodesJson
}
